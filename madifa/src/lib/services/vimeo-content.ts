...
class VimeoContentManager {
  ...
  async syncContentHierarchy(): Promise<ContentHierarchy> {
    try {
      ...
      for (const folder of folders) {
        const videos = await vimeoService.getVideosByFolder(folder.uri.split('/').pop()!)
  ...
  async getContentByCategory(category: string): Promise<VimeoVideo[]> {
    ...
    const folder = this.contentHierarchy!.folders.find(f => f.name.toLowerCase() === category.toLowerCase())
  ...
  async getFeaturedContent(): Promise<VimeoVideo[]> {
    ...
    if (featuredFolder) {
      return vimeoService.getVideosByFolder(featuredFolder.uri.split('/').pop()!)
    }
    return []
  }
}
