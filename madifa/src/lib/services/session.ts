...
export class SessionService {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          expires_at: new Date(Date.now() + this.settings.sessionDuration * 1000).toISOString()
        })
        .select()
        .single()
      ...
      throw createAPIError(500, 'Failed to create session', 'CREATE_SESSION_ERROR', error)
    }
  }
  ...
  async getUserSessions(userId: string): Promise<Session[]> {
    try {
      ...
      throw createAPIError(500, 'Failed to get user sessions', 'GET_SESSIONS_ERROR', error)
    }
  }
  ...
  async deleteSession(sessionId: string): Promise<void> {
    try {
      ...
      throw createAPIError(500, 'Failed to delete session', 'DELETE_SESSION_ERROR', error)
    }
  }
  ...
  async deleteAllUserSessions(userId: string, exceptSessionId?: string): Promise<void> {
    try {
      ...
      throw createAPIError(500, 'Failed to delete user sessions', 'DELETE_SESSIONS_ERROR', error)
    }
  }
  ...
  async cleanupExpiredSessions(): Promise<void> {
    try {
      ...
      throw createAPIError(500, 'Failed to cleanup expired sessions', 'CLEANUP_SESSIONS_ERROR', error)
    }
  }
  ...
  async extendSession(sessionId: string): Promise<void> {
    try {
      ...
      throw createAPIError(500, 'Failed to extend session', 'EXTEND_SESSION_ERROR', error)
    }
  }
