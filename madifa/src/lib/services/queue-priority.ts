...
class QueuePriorityManager {
  ...
  async calculateContentPriority(content: Content, userId: string): Promise<number> {
    const factors = await this.getPriorityFactors(content, userId)
    return this.computePriorityScore(factors)
  }

  private async getPriorityFactors(content: Content, userId: string): Promise<{
  ...
  private async getUserPreferenceFactor(content: Content, userId: string): Promise<number> {
    ...
    const categoryMatch = preferences.preferred_categories.includes(content.category)
  ...
  private async getWatchHistoryFactor(content: Content, userId: string): Promise<number> {
  ...
  private async getExpirationRiskFactor(content: Content): Promise<number> {
    ...
    const daysUntilExpiration = Math.max(0,
      (new Date(metadata.expiration_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
    )
  ...
  async optimizeQueuePriorities(userId: string): Promise<void> {
    ...
    const itemsWithPriority = await Promise.all(
      queueItems.map(async item => ({
        ...item,
        calculatedPriority: await this.calculateContentPriority(item.content, userId)
      }))
    )
    ...
    await downloadQueueManager.reorderQueue(
      sortedItems.map(item => ({
        id: item.id,
        priority: Math.round(item.calculatedPriority * this.PRIORITY_LEVELS.HIGH)
      }))
    )
  }
}
