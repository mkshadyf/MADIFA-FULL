...
export function useVideoKeyboard({
  ...
  const handleKeyPress = useCallback(async (event: KeyboardEvent) => {
    if (!player || event.repeat || event.target instanceof HTMLInputElement) return
    ...
    const volumeStep = isShiftPressedRef.current ? volumeStepRef.current * 2 : volumeStepRef.current
    const seekStep = isShiftPressedRef.current ? seekStepRef.current * 2 : seekStepRef.current
    ...
    try {
      switch (event.key.toLowerCase()) {
        ...
        case 'arrowleft':
          event.preventDefault()
          const [currentTime1] = await Promise.all([
        ...
        case 'arrowright':
          ...
          await player.setCurrentTime(Math.min(currentTime2 + seekStep, duration))
        ...
        case '9':
          ...
          showToast(`Jumped to ${Math.round((targetTime / duration2) * 100)}%`, 'info')
          break
