...
class ContentService {
  ...
  async updateContentMetadata(
    ...
    const { error } = await this.supabase.from('content_metadata').upsert({
  ...
  private async getRecommendationFactors(
    userId: string
  ...
  private calculateCategoryScore(
    ...
    const categoryOverlap = contentCategories.filter(
      c => viewedCategories.includes(c) || preferredCategories.includes(c)
    ).length
  ...
  private async retrieveOfflineContent(
    contentId: string
