...
class QueueAnalyticsManager {
  ...
  async analyzeQueueEfficiency(userId: string): Promise<{
    ...
    const priorityAlignment = await this.calculatePriorityAlignment(
      queueItems,
      userId
    )
  ...
  private calculateTimeEfficiency(
    ...
    const totalTime = queueItems.reduce(
      (sum, item) => sum + (item.content.duration || 0),
      0
    )
    ...
    return (
      weightedTime /
      (totalTime * Math.max(...queueItems.map(item => item.priority)))
    )
  }
