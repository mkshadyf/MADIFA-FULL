...
export async function getSearchMetrics(
  ...
  try {
    ...
    const { data: noResultQueries } = await supabase.rpc(
      'get_no_result_queries',
      {
        start_date: startDate.toISOString(),
        limit_num: 10,
      }
    ...
    const { data: categoryDistribution } = await supabase.rpc(
...
export async function trackSearchSuggestion(
  ...
  try {
    await supabase.from('search_suggestions_analytics').insert({
      query,
