  ...
  const handleSecurityUpdate = async (
    ...
    try {
      const privacyUpdates = {
        view: updates.visibility === 'public' ? 'anybody' : 'nobody',
        download: updates.permissions?.includes('download') || false,
        embed: updates.visibility === 'public' ? 'public' : 'private',
      }
      await vimeoService.updateVideoSecurity(videoId, privacyUpdates)
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {videos.map(video => (
          <VideoCard
            ...
            onThumbnailUpdate={async thumbnailUrl => {
              console.log('Thumbnail update requested:', thumbnailUrl)
            }}
          />
        ))}
      </div>
